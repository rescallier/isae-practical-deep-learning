...
# %%
trainval_dataset.items = trainval_dataset.items[:4]
...
# %%
sliding_window_dense = SlidingWindow(tile_size=64, stride=64, margin_from_bounds=16, discard_background=True)
sliding_window_sparse = SlidingWindow(tile_size=64, stride=64, margin_from_bounds=16, discard_background=False)
...
# %%
train_stratified_sampler = StratifiedSampler(
    nb_tiles_max=400,
...
# %%
test_stratified_sampler = StratifiedSampler(
    nb_tiles_max=100,
    with_replacement=True,
    shuffle=True,
)
...
# %%
train_images, train_labels = np.asarray([i[0]
                                         for i in train_array.items]), np.asarray([i[1] for i in train_array.items])

# %%
test_images, test_labels = np.asarray([i[0] for i in test_array.items]), np.asarray([i[1] for i in test_array.items])
